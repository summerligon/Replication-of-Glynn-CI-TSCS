...
pdf(file = "/workspaces/Replication-of-Glynn-CI-TSCS/output/si-swank.pdf", height = 5, width = 5, pointsize = 11)
...
abline(v = 0, col = "grey")
segments(x0 = local({m <- pout.bt.lag; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1', 1], x1 = local({m <- pout.bt.lag; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',2], y0 = 6)
segments(x0 = local({m <- swank.unw; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',1], x1 = local({m <- swank.unw; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',2], y0 = 4)
segments(x0 = local({m <- swank.rep; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',1], x1 = local({m <- swank.rep; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',2], y0 = 2)
text(x = local({m <- pout.bt.lag; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',2], y = 6, pos = 4, label = "(a) Control for TVCs")
text(x = local({m <- swank.unw; coefs <- coef(m); ses <- sqrt(diag(vcov(m))); z <- qnorm(0.975); ci <- cbind(coefs - z*ses, coefs + z*ses); colnames(ci) <- c("2.5 %","97.5 %"); ci})['btradesum.l1',2], y = 4, pos = 4, label = "(b) Omit TVCs")
